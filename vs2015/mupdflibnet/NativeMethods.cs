...
namespace mupdflibnet
{
    public static class NativeMethods
    {
        ...
        {
            if (ctx != IntPtr.Zero)
                Low.drop_context(ctx);
        }

        public static IntPtr OpenDocument(IntPtr ctx, string filename)
        {
            IntPtr doc = Low.open_document(ctx, filename, System.IO.Path.GetExtension(filename));
            if (doc == IntPtr.Zero)
        ...
        {
            if (doc != IntPtr.Zero)
                Low.drop_document(ctx, doc);
        }

        public static int GetPageCount(IntPtr ctx, IntPtr doc)
        {
            int pagecount = Low.get_pagecount(ctx, doc);
            if (pagecount == 0)
        ...
        {
            IntPtr page = Low.load_page(ctx, doc, pageNumber);
            if (page == IntPtr.Zero)
                throw new NativeException("Failed to load page");
            return page;
        }

        public static SizeF GetPageSize(IntPtr ctx, IntPtr page)
        {
            float width, height;
            Low.get_pagesize(ctx, page, out width, out height);
            if (width == 0 || height == 0)
        ...
        {
            if (page != IntPtr.Zero)
                Low.drop_page(ctx, page);
        }

        public static IntPtr LoadDisplayList(IntPtr ctx, IntPtr page)
        {
            IntPtr list = Low.load_displaylist(ctx, page);
            if (list == IntPtr.Zero)
        ...
        {
            if (0 != Low.run_displaylist(ctx, list, pix, scale, rotation, x0, y0))
        ...
        {
            if (list != IntPtr.Zero)
                Low.drop_displaylist(ctx, list);
        }

        public static IntPtr NewPixmapARGB(IntPtr ctx, int width, int height)
        {
            IntPtr pix = Low.new_pixmap_argb(ctx, width, height);
            if (pix == IntPtr.Zero)
        ...
        {
            int w, h;
            IntPtr data = Low.get_pixmap_data(ctx, pix, out w, out h);
            if (data == IntPtr.Zero)
                throw new NativeException("Failed to get pixmap data");
            return data;
        }
        ...
        public static string GetPagePlainText(IntPtr ctx, IntPtr displayList)
        {
            ...
            try
            {
                buff = Low.load_plaintext_buffer(ctx, displayList);
                if (buff == IntPtr.Zero)
                    throw new NativeException("Failed to load text buffer");
                IntPtr txtPtr = Low.get_string_from_buffer(ctx, buff);
        ...
        public static RectangleF[] SearchForText(IntPtr ctx, IntPtr list, string needle)
        {
            ...
            {
                int hit_count;
                rectPtr = Low.search_for_text(ctx, list, needle, max_hits, out hit_count);
