...
namespace mupdflibnet
{
    public class Page : IDisposable
    {
        ...
        public void CacheDisplayList()
        {
            if (!DisplayListCached)
            {
                ListPtr = NativeMethods.LoadDisplayList(CtxPtr, PagePtr);
                DisplayListCached = true;
            }
        }
        
        public unsafe Bitmap Render32(int destWidth, int destHeight)
        {
            IntPtr list = DisplayListCached ? ListPtr : NativeMethods.LoadDisplayList(CtxPtr, PagePtr);
